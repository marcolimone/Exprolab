...
class ArmorQueryClient(object):
    ...
    def class_b2_ind(self, ind_name, bottom):
        """
        Query the list of all individuals belonging to a class.
        ...
        """
        try:
            res = self._client.call('QUERY', 'CLASS', 'IND', [ind_name, bottom])
    
        except rospy.ServiceException:
            raise ArmorServiceCallError(
                "Service call failed upon querying individuals belonging to class {0}".format(ind_name, bottom))
